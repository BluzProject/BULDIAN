...
exports.sstab = (url, device = 'tablet') => {
	return new Promise((resolve, reject) => {
		 const base = 'https://www.screenshotmachine.com'
		 const param = {
		   url: url,
...
//BY UDIN WA.ME/6289696090800 WM JANGAN DI HAPUS TOD
exports.Ytd = async(url) => {
  ...
  exports.Yt = async(url) => {
	...
	exports.ttblu = async(url) => {
	  let post = {
		url: url
	  }
	  let bluz = await axios.post("https://downloader.bot/api/tiktok/info",post,
	{ headers:{
